...
public class BatchSuggestCourseConfig {
    ...
    public ItemWriter<EmailContentDto> writerForSendSuggestedEmail() {
        return items -> {
            for (EmailContentDto emailContentDto : items) {
                emailService.sendEmail(emailContentDto.getEmail(),
                        "Suggested course", buildEmailContent(emailContentDto));
            }
        };
    }
    ...
    private String buildEmailContent(EmailContentDto emailContentDto) {
        ...
        for (Course course : emailContentDto.getCourses()) {
            courseDetails.append("<div class=\"course\">")
                    .append("<strong>Khóa học:</strong> ").append(course.getTitle()).append("<br>")
                    .append("<strong>Ngày bắt đầu:</strong> ").append(course.getStartDate()).append("<br>")
                    .append("<strong>Số lượng đăng ký tối đa:</strong> ").append(course.getMaxEnrollments()).append("<br>")
                    .append("<strong>Số lượng hiện tại:</strong> ").append(course.getCurrentEnrollments())
                    .append("</div><br>");
        }
